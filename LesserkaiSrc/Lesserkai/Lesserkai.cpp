...
#endif
{
	char buf[10000];
	while (true) {
		// VS2017でx86のリリース版をコンパイルする場合、理由がよくわからないが、
		// fgets()の前にbuf[0] = 0 を呼ばないと、fgets()が正常に動作しない。
		// 最初に送るコマンドが３文字の"usi"であれば、それ以降のコマンドも必ず
		// ３文字以下と判断されてしまう。
		buf[0] = 0;
	...
	while (true) {
		while (true) {
			...
			if (strncmp(buf, "usi", len) == 0) {
				...
				printf("id name Lesserkai 1.4.2");
#ifdef _WIN64
				printf(" x64");
#endif
#ifdef _DEBUG
				...
				// エンジンに設定するパラメータが送られてくるのでそれを設定する。
